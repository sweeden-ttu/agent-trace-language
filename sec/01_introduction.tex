...
\IEEEPARstart{T}{he} contemporary literature on autonomous
software agents is rich in architectural diagrams and short on
formal definitions.  An agent is variously described as ``a
system that perceives and acts'', ``a planner that calls tools'',
or ``an LLM in a loop'', and each definition is stated at a level
of abstraction that admits neither a decision procedure nor a
complexity bound.  The consequence is that two agents with
radically different computational power are routinely compared
as if they belonged to a single class, and verification claims
about agentic systems are correspondingly difficult to falsify.

We are not the first to ask whether the Chomsky hierarchy is the
right scaffold for thinking about this problem.  In particular,
Koohestani, Li, Podkopaev, and Izadi pose the question
\emph{Are agents just automata?}~\cite{koohestani2025automata}
and answer affirmatively, establishing a memory-based equivalence
between simple-reflex agents and finite automata, hierarchical
task-decomposition agents and pushdown automata, and
unrestricted-memory agents and Turing
machines~\cite{turing1936computable}.  We accept their answer.
Their question, however, is the preliminary one.  The question
that does practical work is operational: \emph{given that agents
are automata, how can the Chomsky type of one agent be used to
\textbf{bound} the Chomsky type of another?}  This is the
producer--consumer problem of
Dijkstra~\cite{dijkstra1968cooperating} re-stated in the language
of formal grammars.  The present paper offers a methodology that
answers it in the affirmative, with software verification and
validation techniques inferred into the consumer side and the
idealised theoretical apparatus of automata built into the
producer side.
...
1956--59~\cite{chomsky1956three,chomsky1959certain}.  An agent,
for us, is an effectful machine that, when started on an
...
agent is studied through its traces, then those traces can be
recorded, stored, projected, and checked against explicit language
specifications.  A finite experiment produces finite strings, not a
proof of the full generating language, so we use empirical traces as
validation artifacts and as evidence for modelling hypotheses.  This
places agent trace validation near the V\&V of compilers, parsers,
and protocol implementations~\cite{aho1972theory,hoare1969axiomatic},
while preserving the distinction between observed executions and
language-class theorems.
...
agent, modelled by an idealised recursive-enumeration envelope.  In
practice it is realised as a large language model wrapped in a
tool-calling loop, and the loop is permitted to enumerate
finite-length sequences of tool invocations from the union of the
sub-agents' alphabets.  The Goal-Driven producer has no a priori
knowledge of \emph{what} the goal is; it knows only the
specification of the input domain and the tool catalogue.
...
tests, the column-name checks, and the integrity tests on the
\texttt{train.csv}\,/\,\texttt{test.csv}\,/\,\texttt{sample\_submission.csv}
triple.

The binding is the entirety of our architectural claim:
\begin{quote}
\itshape A single idealised Goal-Driven producer, constrained by a
regular Data-Driven verifier, can yield a validated trace.
\end{quote}
...
verifier-side budget.  We discuss the engineering consequence in
...
$L_0$, given concretely in
\texttt{data/trace\_language\_original.csv}.  The start language
contains twelve sub-agents and their seed action alphabets ---
nothing else.  The Goal-Driven producer, constrained by the
Data-Driven Type-3 verifier, then enumerates a finite trace for the
\textsc{Spaceship Titanic} task instance.  The result is the
observed trace artifact $L_1$, recorded in
\texttt{data/trace\_language\_kaggle.csv}.  $L_1$ contains
...
We use this enumeration as the operational analogue of recursive
enumeration in the agentic setting.  Turing's machines enumerate
strings; our idealised producer enumerates tool-call strings; and
the connection between the emitted strings and the empirical Kaggle
competition is mediated by the Aho--Corasick verifier.
...
Goal-Driven agent is analysed under a \emph{Type-0 abstraction}
or envelope for recursive enumeration; this is not a claim that the
finite implementation realizes a strict Type-0 language.
...
The realised machine is, formally, a finite physical computation,
and under an input-tape accounting can be viewed as a very large
linear-bounded process.  The idealisation is one-way: every trace producible by the realised LBA is also
producible by the idealised Turing machine, and conformance to the verifier language is, by the DFA
construction in Theorem~\ref{thm:closure}, checked in linear time.  The \emph{physical} computation is therefore
far more restricted than the theoretical envelope, and any trace emitted by the bounded machine is contained in the
idealised envelope used for analysis.  This is the standard
idealisation of
...
\item A producer--consumer answer to the
Koohestani--Li--Podkopaev--Izadi question:  a single idealised Goal-Driven producer, intersected with a single Data-Driven Type-3
consumer realised as an Aho--Corasick DFA, suffices to produce a
validated trace while preserving the producer class and keeping
verifier-side conformance checking linear (Section~\ref{sec:chomsky},
Theorem~\ref{thm:closure}).
\item A demonstration that Type-2-style and Type-1-style roles can be
modelled as projections of the observed trace onto stack-discipline
and bounded-workspace substrates respectively
...
Section~\ref{sec:indexed} gives the Aho--Corasick verifier and
records the indexed-grammar interpretation as a future-work
hypothesis.
