...
verification budget of Theorem~\ref{thm:closure}.
...
the verification budget unnecessarily and risk the halting
...
``idealised Type-0 versus realised LBA'' discussion in
...
our framework Aho--Corasick is the canonical example of a
Type-3 construction for keyword-based matching.  The realised
Data-Driven verifier in this paper is instead a pipeline lifecycle
DFA compiled from the 13-state transition table of
\texttt{neurogolf\_dfa\_verifier.py}; it plays the same
architectural role as Aho--Corasick but constrains pipeline-stage
ordering rather than keyword occurrence.
...
verified or refuted by the consumer.  Clarke and
...
set admits no general algorithm.  The Goal-Driven Type-0 producer
in our architecture inherits this undecidability directly: in the
absence of the Type-3 verifier, no algorithm decides whether a
candidate trace lies in the producer's trace language.  The
verifier is the entire reason the system is decidable in
practice.
...
machine-independent measure in which we report the verified
...
as a whole to remain decidable.  The empirical instantiation
(Sections~\ref{sec:expsetup}--\ref{sec:results}) demonstrates the
trace-language account on two tasks with a Type-3 pipeline DFA
as the verifier.  The verifier-expressiveness ladder
(Section~\ref{sec:verifier-ladder}) establishes that Type-3 is the
principled floor: lower than Type-0 (which dissolves soundness)
and more budget-preserving than Type-1 or Type-2.  The empirical
instantiation is reported as data, not as theory, and stands or
falls on its own reproducibility artefacts.
